...
# # 72-Hour Renewable Energy Forecast (Cycle-LSTM) - Improved Model with Benchmarking
...
from tensorflow.keras.layers import Input, Dense, Dropout, LSTM, Add
...
def build_cycle_lstm_model(input_shape, forecast_horizon):
    ...
    lstm_out2 = LSTM(128, return_sequences=False, go_backwards=True)(inputs)
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - Cycle-LSTM')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_cycle_lstm.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_cycle_lstm.png")
        ...
        print(f"{'Model Name:':<25} Cycle-LSTM ({SEASON})")
