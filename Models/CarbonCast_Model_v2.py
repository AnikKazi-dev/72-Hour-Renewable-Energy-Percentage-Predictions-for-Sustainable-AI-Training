...
# # 72-Hour Renewable Energy Forecast (CarbonCast) - V2 Improved Winter Model
# 
# ## Modifications for V2:
# 
# This notebook enhances the original CarbonCast model with several key improvements aimed at boosting predictive accuracy and ensuring robustness:
# 
# 1.  **Stacked LSTM Layers:** The single LSTM layer has been replaced with a stacked architecture of two LSTM layers. This allows the model to learn more complex and hierarchical temporal features from the data.
# 
# 2.  **Bidirectional LSTMs:** `Bidirectional` wrappers have been used for both LSTM layers. This allows the model to process the input sequence in both forward and backward directions, which can lead to improved performance.
# 
# 3.  **Refined Architecture:** The number of units in the LSTM and Dense layers has been adjusted to create a more balanced and efficient model. The LSTM layers now have `64` units, and the Dense layer has `100` units.
# 
...
from tensorflow.keras.layers import Input, Dense, Dropout, Conv1D, BatchNormalization, LSTM, Bidirectional
...
# --- Load and filter data for the selected season ---
...
def build_carboncast_model(input_shape, forecast_horizon):
    ...
    # V2: Stacked Bidirectional LSTM
    lstm_out = Bidirectional(LSTM(64, return_sequences=True))(cnn_out)
...
if X_train.shape[0] > 0:
    solar_model = build_carboncast_model(
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - CarbonCast V2')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_carboncast_v2.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_carboncast_v2.png")
        ...
        print(f"{'Model Name:':<25} CarbonCast ({SEASON} V2)")
