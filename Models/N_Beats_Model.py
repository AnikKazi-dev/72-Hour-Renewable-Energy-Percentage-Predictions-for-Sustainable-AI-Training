...
# # 72-Hour Renewable Energy Forecast (N-BEATS) - Improved Winter Model with Benchmarking
...
if renewable_series_data is not None and not renewable_series_data.empty:
    ...
    if X_seq.shape[0] > 0:
        # N-BEATS expects a 2D input (batch_size, look_back)
...
class NBeatsBlock(Layer):
    def __init__(self, units, theta_dim, num_layers, look_back, forecast_horizon, **kwargs):
...
def build_nbeats_model(look_back, forecast_horizon, num_blocks=3, num_layers=4, units=256, theta_dim=256):
    """
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - N-BEATS')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_nbeats.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                # Reshape for N-BEATS input if necessary
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_nbeats.png")
        ...
        print(f"{'Model Name:':<25} N-BEATS ({SEASON})")
