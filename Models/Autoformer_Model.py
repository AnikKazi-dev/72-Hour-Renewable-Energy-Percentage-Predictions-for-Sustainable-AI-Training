...
# 72-Hour Renewable Energy Forecast (Autoformer) - Seasonal Model with Benchmarking
...
class AutoCorrelation(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.head = self.add_weight(shape=(self.d_model, self.d_model),
                                     initializer='glorot_uniform',
                                     name='autocorr_head')

    def call(self, query, key, value):
        # Simplified Auto-Correlation
        q_fft = tf.signal.rfft(query)
        k_fft = tf.signal.rfft(key)
        corr = q_fft * tf.math.conj(k_fft)
        corr = tf.signal.irfft(corr)
        
        # Apply learnable weight
        output = tf.einsum('bnd,dd->bnd', corr, self.head)
        return output
...
def build_autoformer_model(
    ...
    x = Dense(head_size)(inputs) # Input projection
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - Autoformer')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_autoformer.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_autoformer.png")
        ...
        print(f"{'Model Name:':<25} Autoformer ({SEASON})")
