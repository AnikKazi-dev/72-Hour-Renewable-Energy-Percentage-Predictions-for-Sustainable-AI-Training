...
# # 72-Hour Renewable Energy Forecast (Transformer) - V3 Season-aware Model
# 
# ## Modifications for V3:
# 
# This notebook enhances the V2 Transformer model, building upon its corrected architecture with several key improvements aimed at boosting predictive power:
# 
...
# 
# 2.  **L2 Regularization**: To prevent overfitting with the increased model capacity, L2 regularization (`kernel_regularizer`) has been added to all `Dense` and `MultiHeadAttention` layers. This encourages the model to learn smaller, more generalizable weights.
# 
# 3.  **Increased Model Capacity**: The model's capacity has been significantly increased to capture more complex patterns, reverting to the more powerful configuration of the V1 model but now with the crucial positional embeddings from V2. The number of transformer blocks is increased from 2 to 4, attention heads from 2 to 4, and the head size from 128 to 256.
# 
...
class TokenAndPositionEmbedding(tf.keras.layers.Layer):
    def __init__(self, maxlen, embed_dim, l2_reg):
        super(TokenAndPositionEmbedding, self).__init__()
        self.pos_emb = Embedding(input_dim=maxlen, output_dim=embed_dim)
        self.dense_proj = Dense(embed_dim, kernel_regularizer=l2_reg)
...
def transformer_encoder(inputs, head_size, num_heads, ff_dim, dropout, l2_reg):
    # Attention and Normalization
    x = LayerNormalization(epsilon=1e-6)(inputs)
    x = MultiHeadAttention(
        key_dim=head_size, num_heads=num_heads, dropout=dropout, kernel_regularizer=l2_reg
    )(x, x)
    x = Dropout(dropout)(x)
    res = x + inputs
    ...
    x = Dense(ff_dim, activation="relu", kernel_regularizer=l2_reg)(x)
    x = Dropout(dropout)(x)
    x = Dense(inputs.shape[-1], kernel_regularizer=l2_reg)(x)
    return x + res
...
def build_transformer_model(
    ...
    x = TokenAndPositionEmbedding(input_shape[0], head_size * num_heads, l2_reg)(inputs)
...
if X_train.shape[0] > 0:
    solar_model = build_transformer_model(
        ...
        head_size=256, # V3: Increased capacity
        num_heads=4,   # V3: Increased capacity
        ff_dim=4,
        num_transformer_blocks=4, # V3: Increased capacity
        mlp_units=[256, 128], # V3: Deeper MLP Head
        mlp_dropout=0.4,
        dropout=0.25,
    )
else:
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - Transformer V3')
    plt.ylabel('Loss (Huber)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_transformer_v3.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_transformer_v3.png")
            
        # --- BENCHMARKING REPORT ---
        print("\n\n--- BENCHMARKING REPORT (V3) ---")
        total_params = solar_model.count_params()
        model_size_mb = total_params * 4 / (1024**2)
        print(f"{'Model Name:':<25} Transformer ({SEASON} V3)")
