...
# # 72-Hour Renewable Energy Forecast (PatchTST) - V2 Improved Winter Model
# 
# ## Modifications for V2:
# 
# This notebook enhances the original PatchTST model with several key improvements aimed at boosting predictive accuracy and ensuring robustness:
# 
...
# 
# 2.  **Enhanced Patching:** The `tf.space_to_depth` patching has been replaced with a more robust `Patching` layer. This provides more flexibility and control over the patching process, which is a core component of the PatchTST architecture.
# 
# 3.  **Positional Embedding:** A `TokenAndPositionEmbedding` layer has been introduced. The standard PatchTST architecture requires explicit positional information to understand the sequence order. This new layer adds positional embeddings to the input, ensuring the model is aware of the temporal dependencies in the data.
# 
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - PatchTST V2')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_patchtst_v2.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_patchtst_v2.png")
        ...
        print(f"{'Model Name:':<25} PatchTST ({SEASON} V2)")
