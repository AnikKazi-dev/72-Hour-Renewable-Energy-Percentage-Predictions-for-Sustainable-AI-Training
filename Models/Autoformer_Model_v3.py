...
# 72-Hour Renewable Energy Forecast (Autoformer) - V3 Seasonal Model
# 
# ## Modifications for V3:
# 
# This notebook enhances the V2 Autoformer model with several key improvements aimed at boosting predictive accuracy and ensuring robustness:
# 
...
# 
# 2.  **L2 Regularization**: L2 regularization has been added to the `Conv1D` and `Dense` layers in the model. L2 regularization, also known as weight decay, penalizes large weights in the model. This helps to prevent overfitting by encouraging the model to learn smaller and more distributed weights, which can lead to better generalization on unseen data.
# 
# 3.  **Increased Model Complexity**: To potentially capture more intricate patterns in the data, the model's complexity has been slightly increased. The number of `encoder_layers` is increased from `2` to `3`, and the `ff_dim` (feed-forward dimension) from `128` to `256`. This gives the model more capacity to learn from the time series data.
...
try:
    ...
    # Filter the loaded data to the selected season
...
def autoformer_encoder(inputs, head_size, num_heads, ff_dim, dropout=0):
    ...
    x = Conv1D(filters=ff_dim, kernel_size=1, activation="relu", kernel_regularizer=tf.keras.regularizers.l2(0.001))(x) # V3: Added L2 regularization
    x = Dropout(dropout)(x)
    x = Conv1D(filters=inputs.shape[-1], kernel_size=1, kernel_regularizer=tf.keras.regularizers.l2(0.001))(x) # V3: Added L2 regularization
    return x + res
...
def build_autoformer_model(
    ...
    for dim in mlp_units:
        x = Dense(dim, activation="relu", kernel_regularizer=tf.keras.regularizers.l2(0.001))(x) # V3: Added L2 regularization
...
if X_train.shape[0] > 0:
    solar_model = build_autoformer_model(
        ...
        ff_dim=256, # V3: Increased complexity
        num_encoder_layers=3, # V3: Increased complexity
        mlp_units=[256],
        mlp_dropout=0.3,
        dropout=0.2,
    )
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - Autoformer V3')
    plt.ylabel('Loss (Huber)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_autoformer_v3.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_autoformer_v3.png")
        ...
        print(f"{'Model Name:':<25} Autoformer ({SEASON} V3)")
