...
# # 72-Hour Renewable Energy Forecast (Hybrid CNN-CycleLSTM-Attention) - Improved Winter Model with Benchmarking
...
from tensorflow.keras.layers import Input, Dense, Dropout, Conv1D, BatchNormalization, LSTM, Add, Multiply, Permute, Reshape, GlobalAveragePooling1D
...
def attention_block(inputs):
    # Permute and Reshape for Dense layer
    a = Permute((2, 1))(inputs)
    a = Dense(inputs.shape[1], activation='softmax')(a)
    # Permute back
    a = Permute((2, 1))(a)
    # Multiply with original inputs
    output_attention_mul = Multiply()([inputs, a])
    return output_attention_mul

def build_hybrid_model(input_shape, forecast_horizon):
    ...
    # CycleLSTM part
    lstm_out1 = LSTM(128, return_sequences=True)(cnn_out)
    lstm_out2 = LSTM(128, return_sequences=True, go_backwards=True)(cnn_out)
    lstm_out = Add()([lstm_out1, lstm_out2])
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - Hybrid CNN-CycleLSTM-Attention')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_hybrid.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_hybrid.png")

        # --- BENCHMARKING REPORT ---
        print("\n\n--- BENCHMARKING REPORT ---")
        total_params = solar_model.count_params()
        model_size_mb = total_params * 4 / (1024**2)
        print(f"{'Model Name:':<25} Hybrid CNN-CycleLSTM-Attention ({SEASON})")
