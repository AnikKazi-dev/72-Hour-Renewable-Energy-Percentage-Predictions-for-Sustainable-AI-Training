...
# # 72-Hour Renewable Energy Forecast (CarbonCast) - V3 Improved Winter Model
# 
# ## Modifications for V3:
# 
# This notebook enhances the V2 CarbonCast model with several key improvements aimed at boosting predictive accuracy and ensuring robustness:
# 
...
def build_carboncast_model(input_shape, forecast_horizon):
    ...
    # CNN part
    cnn_out = Conv1D(filters=128, kernel_size=3, activation='relu', padding='same', kernel_regularizer=tf.keras.regularizers.l2(0.001))(inputs) # V3: Increased filters and added L2 regularization
    cnn_out = BatchNormalization()(cnn_out)
    cnn_out = Dropout(0.2)(cnn_out)
    
    # V2: Stacked Bidirectional LSTM
    lstm_out = Bidirectional(LSTM(128, return_sequences=True, kernel_regularizer=tf.keras.regularizers.l2(0.001)))(cnn_out) # V3: Increased units and added L2 regularization
    lstm_out = Bidirectional(LSTM(128, return_sequences=False, kernel_regularizer=tf.keras.regularizers.l2(0.001)))(lstm_out) # V3: Increased units and added L2 regularization
    
    # Dense layers
    x = Dense(128, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))(lstm_out) # V3: Increased units and added L2 regularization
    x = Dropout(0.3)(x)
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - CarbonCast V3')
    plt.ylabel('Loss (Huber)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_carboncast_v3.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_carboncast_v3.png")
        ...
        print(f"{'Model Name:':<25} CarbonCast ({SEASON} V3)")
