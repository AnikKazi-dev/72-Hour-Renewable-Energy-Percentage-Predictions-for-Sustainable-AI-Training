...
# # 72-Hour Renewable Energy Forecast (DLinear) - V2 Improved Model
# 
# ## Modifications for V2:
# 
# This notebook enhances the original DLinear model with several key improvements aimed at boosting predictive accuracy and ensuring robustness:
# 
# 1.  **Moving Average Kernel:** A `MovingAverage` layer has been introduced to explicitly capture the trend component of the time series. This is a core component of the DLinear architecture and helps the model to better distinguish between trend and seasonal patterns.
# 
# 2.  **Individual Component Outputs:** The model has been modified to output the seasonal and trend components separately before adding them together. This allows for better interpretability and a more robust decomposition of the time series.
# 
# 3.  **Refined Architecture:** The number of units in the Dense layers has been adjusted to create a more balanced and efficient model. The seasonal and trend components now each have `512` units.
# 
...
if renewable_series_data is not None and not renewable_series_data.empty:
    ...
    if X_seq.shape[0] > 0:
        # DLinear expects a 2D input (batch_size, look_back)
...
# V2: Moving Average Layer
class MovingAverage(Layer):
    def __init__(self, kernel_size, **kwargs):
        super(MovingAverage, self).__init__(**kwargs)
        self.avg_pool = tf.keras.layers.AvgPool1D(pool_size=kernel_size, strides=1, padding='same')
...
def build_dlinear_model(input_shape, forecast_horizon, kernel_size=25):
    inputs = Input(shape=input_shape)
    
    # Trend part
    trend_input = MovingAverage(kernel_size)(inputs)
    trend_output = Dense(forecast_horizon)(trend_input)
    
    # Seasonal part
    seasonal_input = inputs - trend_input
    seasonal_output = Dense(forecast_horizon)(seasonal_input)
    
    # Combine
    outputs = Add()([seasonal_output, trend_output])
...
if X_train.shape[0] > 0:
    solar_model = build_dlinear_model(
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - DLinear V2')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_dlinear_v2.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                historical_input_inversed = scaler.inverse_transform(X_test[sample_index].reshape(1, -1)).flatten()
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_dlinear_v2.png")
        ...
        print(f"{'Model Name:':<25} DLinear ({SEASON} V2)")
