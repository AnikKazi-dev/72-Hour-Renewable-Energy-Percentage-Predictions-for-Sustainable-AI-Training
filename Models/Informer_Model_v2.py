...
# # 72-Hour Renewable Energy Forecast (Informer) - V2 Improved Winter Model
# 
# ## Modifications for V2:
# 
# This notebook enhances the original Informer model with several key improvements aimed at boosting predictive accuracy and ensuring robustness:
# 
# 1.  **Architectural Refinement:** The model's complexity has been reduced to better suit the univariate time series data. The number of encoder layers is decreased from `3` to `2`, the number of attention heads from `4` to `2`, and the `head_size` from `128` to `64`. This creates a more lightweight and efficient architecture, reducing the risk of overfitting and improving generalization.
# 
# 2.  **Positional Embedding:** A `TokenAndPositionEmbedding` layer has been introduced. The standard Informer architecture requires explicit positional information to understand the sequence order. This new layer adds positional embeddings to the input, ensuring the model is aware of the temporal dependencies in the data.
# 
# 3.  **Dynamic Learning Rate Scheduling:** The `ReduceLROnPlateau` callback has been added to the training process. This allows the learning rate to be adjusted dynamically based on the validation loss, enabling the model to converge more effectively and achieve a better final result.
...
from tensorflow.keras.layers import Input, Dense, Dropout, LayerNormalization, MultiHeadAttention, Add, GlobalAveragePooling1D, Conv1D, Embedding
...
def informer_encoder(inputs, head_size, num_heads, ff_dim, dropout=0):
...
def build_informer_model(
    ...
    x = TokenAndPositionEmbedding(input_shape[0], head_size * num_heads)(inputs)
    for _ in range(num_encoder_layers):
        x = informer_encoder(x, head_size, num_heads, ff_dim, dropout)
...
if X_train.shape[0] > 0:
    solar_model = build_informer_model(
        input_shape=(LOOK_BACK, N_FEATURES),
        head_size=64, # V2: Reduced complexity
        num_heads=2,   # V2: Reduced complexity
        ff_dim=128,
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - Informer V2')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_informer_v2.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_informer_v2.png")
            
            # --- BENCHMARKING REPORT ---
            print("\n\n--- BENCHMARKING REPORT (V2) ---")
            total_params = solar_model.count_params()
            model_size_mb = total_params * 4 / (1024**2)
            print(f"{'Model Name:':<25} Informer ({SEASON} V2)")
