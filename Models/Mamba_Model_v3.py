...
# # 72-Hour Renewable Energy Forecast (Mamba) - V3 Improved Winter Model
# 
# ## Modifications for V3:
# 
# This notebook enhances the V2 Mamba model with several key improvements aimed at boosting predictive accuracy and robustness:
# 
...
# 
# 2.  **L2 Regularization**: To prevent overfitting, L2 regularization (`kernel_regularizer`) has been added to all `Dense` and `Conv1D` layers within the Mamba blocks and the final MLP head. This encourages the model to learn smaller, more generalizable weights.
# 
# 3.  **Increased Model Capacity**: The model's capacity has been significantly increased to capture more complex patterns in the data. The number of Mamba layers is increased from 2 to 4, and the internal model dimension (`d_model`) is increased from 64 to 128.
# 
...
class MambaBlock(tf.keras.layers.Layer):
    def __init__(self, d_model, d_state, d_conv, expand, **kwargs):
        ...
        self.in_proj = Dense(self.expand * self.d_model * 2, use_bias=False, kernel_regularizer=l2_reg)
        self.conv1d = Conv1D(filters=self.expand * self.d_model, kernel_size=d_conv, padding='causal', groups=self.expand * self.d_model, kernel_regularizer=l2_reg)
        self.out_proj = Dense(self.d_model, use_bias=False, kernel_regularizer=l2_reg)
...
def build_mamba_model(input_shape, forecast_horizon, d_model=128, d_state=16, d_conv=4, expand=2, num_layers=4, mlp_units=[512, 256], mlp_dropout=0.3):
    l2_reg = tf.keras.regularizers.l2(0.001)
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - Mamba V3')
    plt.ylabel('Loss (Huber)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_mamba_v3.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_mamba_v3.png")
            
            # --- BENCHMARKING REPORT ---
            print("\n\n--- BENCHMARKING REPORT (V3) ---")
            total_params = solar_model.count_params()
            model_size_mb = total_params * 4 / (1024**2)
            print(f"{'Model Name:':<25} Mamba ({SEASON} V3)")
