...
# # 72-Hour Renewable Energy Forecast (Mamba) - Improved Winter Model with Benchmarking
...
def build_mamba_model(input_shape, forecast_horizon, d_model=128, d_state=16, d_conv=4, expand=2, num_layers=3, mlp_units=[256], mlp_dropout=0.3):
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - Mamba')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_mamba.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_mamba.png")
            
            # --- BENCHMARKING REPORT ---
            print("\n\n--- BENCHMARKING REPORT ---")
            total_params = solar_model.count_params()
            model_size_mb = total_params * 4 / (1024**2)
            print(f"{'Model Name:':<25} Mamba ({SEASON})")
