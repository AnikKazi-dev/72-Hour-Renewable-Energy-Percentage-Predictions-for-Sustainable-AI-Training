...
# # 72-Hour Renewable Energy Forecast (Hybrid CNN-CycleLSTM-Attention) - V2 Improved Winter Model
# 
# ## Modifications for V2:
# 
# This notebook introduces several key enhancements to the original hybrid model to improve its performance and robustness:
# 
# 1.  **Refined LSTM Layer:** The number of units in the LSTM layers has been reduced from `128` to `64`. This creates a more streamlined architecture that is less prone to overfitting and more computationally efficient, which is better suited for univariate time series data.
# 
# 2.  **Enhanced Attention Mechanism:** The custom attention block has been replaced with the standard `MultiHeadAttention` layer from Keras. This provides a more powerful mechanism for the model to weigh the importance of different time steps, allowing it to capture more complex temporal patterns.
# 
# 3.  **Standardized Bidirectional LSTM:** The manual implementation of the CycleLSTM has been refactored to use the `tf.keras.layers.Bidirectional` wrapper with `merge_mode='sum'`. This improves code readability and aligns with standard Keras practices while maintaining the same architectural logic.
# 
# 4.  **Dynamic Learning Rate Scheduling:** The `ReduceLROnPlateau` callback has been added. This allows the learning rate to be adjusted dynamically during training, helping the model to converge more effectively and find a better minimum in the loss landscape.
...
from tensorflow.keras.layers import Input, Dense, Dropout, Conv1D, BatchNormalization, LSTM, Add, Bidirectional, MultiHeadAttention, GlobalAveragePooling1D, LayerNormalization
...
try:
    ...
    # Filter the loaded data to ensure it only contains the selected season months
...
def build_hybrid_model(input_shape, forecast_horizon, num_heads=2):
    ...
    # V2: Standardized Bidirectional LSTM with reduced units
    lstm_out = Bidirectional(LSTM(64, return_sequences=True), merge_mode='sum')(cnn_out)
    
    # V2: Enhanced Attention part with MultiHeadAttention
    attention_out = MultiHeadAttention(num_heads=num_heads, key_dim=lstm_out.shape[-1], dropout=0.1)(lstm_out, lstm_out)
    attention_out = Add()([lstm_out, attention_out]) # Add & Norm
...
if X_train.shape[0] > 0:
    solar_model = build_hybrid_model(
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - Hybrid V2')
    ...
    if X_test.shape[0] > 0:
        ...
        print(f"{'Model Name:':<25} Hybrid CNN-CycleLSTM-Attention ({SEASON} V2)")
