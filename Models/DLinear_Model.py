...
# # 72-Hour Renewable Energy Forecast (DLinear) - Improved Model with Benchmarking
...
def build_dlinear_model(input_shape, forecast_horizon):
    inputs = Input(shape=input_shape)
    
    # Seasonal part
    seasonal_output = Dense(forecast_horizon)(inputs)
    
    # Trend part
    trend_output = Dense(forecast_horizon)(inputs)
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - DLinear')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_dlinear.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_dlinear.png")
        ...
        print(f"{'Model Name:':<25} DLinear ({SEASON})")
