...
# # 72-Hour Renewable Energy Forecast (EnsembleCI) - V2 Improved Winter Model
# 
# ## Modifications for V2:
# 
# This notebook enhances the original EnsembleCI model with several key improvements aimed at boosting predictive accuracy and ensuring robustness:
# 
# 1.  **Stacked Bidirectional LSTMs:** The single LSTM layers in both sub-models have been replaced with stacked `Bidirectional(LSTM)` layers. This allows the models to learn more complex and hierarchical temporal features from the data.
# 
# 2.  **Refined Architecture:** The number of units in the LSTM and Dense layers has been adjusted to create a more balanced and efficient model. The LSTM layers now have `64` units, and the Dense layer has `100` units.
# 
# 3.  **Shared Feature Extractor:** The model has been refactored to use a shared feature extractor for both the CNN and LSTM branches. This allows the model to learn a more robust representation of the input data and reduces the number of parameters.
# 
...
from tensorflow.keras.layers import Input, Dense, Dropout, Conv1D, BatchNormalization, LSTM, Add, Concatenate, Bidirectional
...
def build_ensemble_model(input_shape, forecast_horizon):
    inputs = Input(shape=input_shape)
    
    # Shared Feature Extractor
    shared_cnn = Conv1D(filters=64, kernel_size=3, activation='relu', padding='same')(inputs)
    shared_cnn = BatchNormalization()(shared_cnn)
    shared_cnn = Dropout(0.2)(shared_cnn)
    
    # Model 1: CNN-LSTM
    cnn_lstm_out = Bidirectional(LSTM(64, return_sequences=True))(shared_cnn)
    cnn_lstm_out = Bidirectional(LSTM(64, return_sequences=False))(cnn_lstm_out)
    ...
    # Ensemble
    concatenated = Concatenate()([cnn_lstm_out, lstm_out])
...
if X_train.shape[0] > 0:
    solar_model = build_ensemble_model(
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - EnsembleCI V2')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_ensembleci_v2.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_ensembleci_v2.png")

        # --- BENCHMARKING REPORT ---
        print("\n\n--- BENCHMARKING REPORT (V2) ---")
        total_params = solar_model.count_params()
        model_size_mb = total_params * 4 / (1024**2)
        print(f"{'Model Name:':<25} EnsembleCI ({SEASON} V2)")
