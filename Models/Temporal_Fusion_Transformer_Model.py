...
# # 72-Hour Renewable Energy Forecast (Temporal Fusion Transformer) - Season-aware Model with Benchmarking
...
from tensorflow.keras.layers import Input, Dense, Dropout, LayerNormalization, MultiHeadAttention, Add, GlobalAveragePooling1D, Concatenate, LSTM
...
def gated_linear_unit(inputs):
    x = Dense(inputs.shape[-1], activation='sigmoid')(inputs)
    return Add()([x, inputs])
...
def build_tft_model(input_shape, forecast_horizon, d_model=128, num_heads=4, d_ff=256, dropout=0.1):
    ...
    # Static Enrichment
    static_input = Input(shape=(1,)) # Dummy static input
    static_embedding = Dense(d_model)(static_input)
    ...
    grn_out = gated_linear_unit(lstm_out)
    ...
    grn_out_2 = gated_linear_unit(attention_out)
    ...
    # Dummy static input for model creation
    dummy_static_input = np.zeros((X_train.shape[0], 1))
    
    model = Model(inputs=[inputs, static_input], outputs=outputs)
...
if solar_model and X_train.shape[0] > 0:
    ...
    # Dummy static input for training
    dummy_static_train = np.zeros((X_train.shape[0], 1))
    dummy_static_valid = np.zeros((X_valid.shape[0], 1))
    
    history = solar_model.fit([X_train, dummy_static_train], y_train,
                              validation_data=([X_valid, dummy_static_valid], y_valid),
                              epochs=500,
                              batch_size=32,
                              verbose=1,
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - Temporal Fusion Transformer')
    ...
    if X_test.shape[0] > 0:
        ...
        dummy_static_test = np.zeros((X_test.shape[0], 1))
        y_pred_scaled = solar_model.predict([X_test, dummy_static_test])
        ...
        print(f"{'Model Name:':<25} Temporal Fusion Transformer ({SEASON})")
