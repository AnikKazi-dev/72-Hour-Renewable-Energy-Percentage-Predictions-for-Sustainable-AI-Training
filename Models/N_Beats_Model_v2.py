...
# # 72-Hour Renewable Energy Forecast (N-BEATS) - V2 Improved Winter Model
# 
# ## Modifications for V2:
# 
# This notebook is an enhanced version of the original N-BEATS model. The following key improvements have been implemented to boost predictive accuracy and model robustness:
# 
# 1.  **Architectural Refinement:** The number of units in the dense layers of each N-BEATS block has been reduced from `256` to `128`. The original model was quite large for a univariate problem, making it susceptible to overfitting. A smaller, more streamlined architecture encourages better generalization, leading to improved performance on unseen data.
# 
# 2.  **Dropout for Regularization:** A `Dropout(0.2)` layer has been added within each N-BEATS block. Dropout is a powerful regularization technique that randomly deactivates a fraction of neurons during training. This prevents the model from becoming overly reliant on specific neurons and forces it to learn more robust and distributed representations, reducing the risk of overfitting.
# 
# 3.  **Dynamic Learning Rate Scheduling:** The `ReduceLROnPlateau` callback has been integrated into the training process. Instead of using a fixed learning rate, this callback monitors the validation loss and automatically reduces the learning rate when it plateaus. This allows the model to make large adjustments early in training and finer, more precise updates as it converges on a solution, often leading to a better final model.
...
from tensorflow.keras.layers import Input, Dense, Dropout, Lambda, Add, Layer
...
# N-BEATS Block (V2 with Dropout)
class NBeatsBlock(Layer):
    def __init__(self, units, theta_dim, num_layers, look_back, forecast_horizon, dropout_rate=0.2, **kwargs):
        ...
        self.fc_layers = [Dense(units, activation='relu') for _ in range(num_layers)]
        self.dropout_layers = [Dropout(self.dropout_rate) for _ in range(num_layers)]
        
        self.theta_f = Dense(theta_dim, activation='linear', name='theta_f')
        self.theta_b = Dense(theta_dim, activation='linear', name='theta_b')
        
        # Generic basis functions
        self.g_f = Dense(self.forecast_horizon, activation='linear')
        self.g_b = Dense(self.look_back, activation='linear')
    ...
    def call(self, inputs, training=False):
        ...
        theta_f_payload = self.theta_f(x)
        theta_b_payload = self.theta_b(x)
        
        forecast = self.g_f(theta_f_payload)
        backcast = self.g_b(theta_b_payload)
        
        return backcast, forecast

def build_nbeats_model(look_back, forecast_horizon, num_blocks=3, num_layers=4, units=128, theta_dim=128):
    """
    Builds the N-BEATS model (V2 with smaller architecture).
    """
    input_layer = Input(shape=(look_back,), name="input_sequence")
    ...
    for i in range(num_blocks):
        with tf.name_scope(f"NBeatsBlock_{i}"):
            nbeats_block = NBeatsBlock(units, theta_dim, num_layers, look_back, forecast_horizon)
            backcast, forecast = nbeats_block(residuals)
            
            residuals = Add()([residuals, -backcast])
            forecasts.append(forecast)

    # Sum up the forecasts from each block
    final_forecast = Add()(forecasts)
...
if X_train.shape[0] > 0:
    solar_model = build_nbeats_model(LOOK_BACK, FORECAST_HORIZON)
else:
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - N-BEATS V2')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_nbeats_v2.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                historical_input_reshaped = X_test[sample_index].reshape(1, -1)
                historical_input_inversed = scaler.inverse_transform(historical_input_reshaped).flatten()
                ...
                plt.plot(time_axis_input, historical_input_inversed, label='Historical Input', marker='o', linestyle=':', color='gray', alpha=0.7)
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_nbeats_v2.png")
        ...
        print(f"{'Model Name:':<25} N-BEATS ({SEASON} V2)")
