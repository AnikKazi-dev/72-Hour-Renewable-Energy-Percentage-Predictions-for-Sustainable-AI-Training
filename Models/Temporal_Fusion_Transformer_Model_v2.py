...
# # 72-Hour Renewable Energy Forecast (Temporal Fusion Transformer) - V2 Season-aware Model
# 
# ## Modifications for V2:
# 
# This notebook refines the Temporal Fusion Transformer (TFT) model with several key improvements to enhance its performance and simplify its architecture for the univariate forecasting task:
# 
# 1.  **Architectural Simplification:** The model's complexity has been reduced to better fit the univariate data. The internal dimension (`d_model`) is decreased from `128` to `64`, and the number of attention heads is reduced from `4` to `2`. This creates a more efficient and less overfitting-prone model.
# 
# 2.  **Corrected Gating Mechanism:** The `gated_linear_unit` function has been corrected to use element-wise multiplication instead of addition. This is the standard implementation of a Gated Linear Unit (GLU), which acts as a proper filter or gate for information flow.
# 
# 3.  **Removal of Dummy Static Features:** The original model included a dummy static input, which added unnecessary complexity for a purely univariate problem. This has been completely removed, simplifying the model's inputs and architecture to focus solely on the temporal dynamics.
# 
# 4.  **Dynamic Learning Rate Scheduling:** The `ReduceLROnPlateau` callback has been added to the training process. This allows the learning rate to be adjusted dynamically based on the validation loss, leading to more stable training and a better final model.
...
def gated_linear_unit(inputs):
    x_sigmoid = Dense(inputs.shape[-1], activation='sigmoid')(inputs)
    x_linear = Dense(inputs.shape[-1], activation='linear')(inputs)
    return Multiply()([x_sigmoid, x_linear])

def gated_residual_network(inputs, d_model, dropout_rate):
    x = Dense(d_model, activation='elu')(inputs)
    x = Dense(d_model)(x)
    x = Dropout(dropout_rate)(x)
    gated_x = gated_linear_unit(x)
    return Add()([inputs, gated_x])

def build_tft_model(input_shape, forecast_horizon, d_model=64, num_heads=2, d_ff=128, dropout=0.1):
    inputs = Input(shape=input_shape)
    
    # Variable Selection Network (simplified for univariate)
    vsn_out = Dense(d_model)(inputs)
    
    # LSTM Encoder
    lstm_out = LSTM(d_model, return_sequences=True)(vsn_out)
    lstm_out = Add()([lstm_out, vsn_out]) # Add residual connection
    lstm_out = LayerNormalization()(lstm_out)
    
    # Gated Residual Network
    grn_out = gated_residual_network(lstm_out, d_model, dropout)
    
    # Self-Attention
    attention_out = MultiHeadAttention(num_heads=num_heads, key_dim=d_model, dropout=dropout)(grn_out, grn_out)
    attention_out = LayerNormalization(epsilon=1e-6)(Add()([attention_out, grn_out]))
    
    # Another Gated Residual Network
    grn_out_2 = gated_residual_network(attention_out, d_model, dropout)
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - TFT V2')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_tft_v2.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_tft_v2.png")
            
        # --- BENCHMARKING REPORT ---
        print("\n\n--- BENCHMARKING REPORT (V2) ---")
        total_params = solar_model.count_params()
        model_size_mb = total_params * 4 / (1024**2)
        print(f"{'Model Name:':<25} Temporal Fusion Transformer ({SEASON} V2)")
