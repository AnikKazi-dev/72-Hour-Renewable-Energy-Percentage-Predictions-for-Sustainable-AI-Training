...
# # 72-Hour Renewable Energy Forecast (PatchTST) - Improved Winter Model with Benchmarking
...
from tensorflow.keras.layers import Input, Dense, Dropout, LayerNormalization, MultiHeadAttention, Add, GlobalAveragePooling1D, Conv1D, Reshape, Permute
...
def build_patchtst_model(
    ...
    patch_dim = patch_size * input_shape[1]
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - PatchTST')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_patchtst.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_patchtst.png")
        ...
        print(f"{'Model Name:':<25} PatchTST ({SEASON})")
