...
# # 72-Hour Renewable Energy Forecast (EnsembleCI) - Improved Winter Model with Benchmarking
...
from tensorflow.keras.layers import Input, Dense, Dropout, Conv1D, BatchNormalization, LSTM, Add, Concatenate
...
def build_ensemble_model(input_shape, forecast_horizon):
    ...
    cnn_out = Conv1D(filters=64, kernel_size=3, activation='relu', padding='same')(inputs1)
    cnn_out = BatchNormalization()(cnn_out)
    cnn_out = LSTM(128, return_sequences=False)(cnn_out)
    model1 = Model(inputs=inputs1, outputs=cnn_out)

    # Model 2: LSTM
    inputs2 = Input(shape=input_shape)
    lstm_out = LSTM(128, return_sequences=False)(inputs2)
    model2 = Model(inputs=inputs2, outputs=lstm_out)
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - EnsembleCI')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_ensembleci.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_ensembleci.png")

        # --- BENCHMARKING REPORT ---
        print("\n\n--- BENCHMARKING REPORT ---")
        total_params = solar_model.count_params()
        model_size_mb = total_params * 4 / (1024**2)
        print(f"{'Model Name:':<25} EnsembleCI ({SEASON})")
