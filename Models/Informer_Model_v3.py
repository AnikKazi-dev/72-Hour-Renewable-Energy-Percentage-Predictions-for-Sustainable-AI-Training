...
# # 72-Hour Renewable Energy Forecast (Informer) - V3 Improved Winter Model
# 
# ## Modifications for V3:
# 
# This notebook builds upon the V2 Informer model, integrating further enhancements for performance and stability:
# 
# 1.  **Huber Loss Function**: Inheriting from other V3 models, the loss function is switched from `Mean Absolute Error` to `Huber` loss. This provides robustness against outliers in the dataset, potentially leading to a more generalized model.
# 
# 2.  **L2 Regularization**: To combat overfitting, L2 regularization (`kernel_regularizer`) has been systematically added to all `MultiHeadAttention`, `Conv1D`, and `Dense` layers. This penalizes large weights, encouraging the model to learn simpler patterns.
# 
# 3.  **Increased Model Capacity**: The model's capacity has been increased from the V2 version to better capture complex temporal dynamics. The number of encoder layers is increased from 2 to 3, the number of attention heads from 2 to 4, and the feed-forward dimension (`ff_dim`) from 128 to 256.
# 
# 4.  **Deeper MLP Head**: The final Multi-Layer Perceptron (MLP) head has been made deeper (from a single `Dense(256)` layer to `Dense(512)` -> `Dense(256)`). This allows the model to learn more complex mappings from the feature representation to the final forecast.
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - Informer V3')
    plt.ylabel('Loss (Huber)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_informer_v3.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_informer_v3.png")
            
            # --- BENCHMARKING REPORT ---
            print("\n\n--- BENCHMARKING REPORT (V3) ---")
            total_params = solar_model.count_params()
            model_size_mb = total_params * 4 / (1024**2)
            print(f"{'Model Name:':<25} Informer ({SEASON} V3)")
