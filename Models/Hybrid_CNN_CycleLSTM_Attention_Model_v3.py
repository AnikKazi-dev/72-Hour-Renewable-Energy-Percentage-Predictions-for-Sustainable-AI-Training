...
# # 72-Hour Renewable Energy Forecast (Hybrid CNN-CycleLSTM-Attention) - V3 Improved Winter Model
# 
# ## Modifications for V3:
# 
# This notebook enhances the V2 Hybrid model with several key improvements aimed at boosting predictive accuracy and robustness:
# 
...
# 
# 3.  **Increased Model Capacity and Deeper Architecture**: To potentially capture more intricate patterns, the model's capacity has been increased. The number of filters/units in the `Conv1D` and `LSTM` layers are increased from `64` to `128`. The number of heads in the `MultiHeadAttention` layer is increased from `2` to `4`. The final dense head has also been made deeper (`Dense(256)` -> `BN` -> `Dense(128)`) to allow for more complex feature extraction before the final prediction.
...
def build_hybrid_model(input_shape, forecast_horizon, num_heads=4):
    inputs = Input(shape=input_shape)
    l2_reg = tf.keras.regularizers.l2(0.001)
    
    # CNN part
    cnn_out = Conv1D(filters=128, kernel_size=3, activation='relu', padding='same', kernel_regularizer=l2_reg)(inputs) # V3: Increased capacity & L2 reg
    cnn_out = BatchNormalization()(cnn_out)
    cnn_out = Dropout(0.2)(cnn_out)
    
    # V2: Standardized Bidirectional LSTM with reduced units
    lstm_out = Bidirectional(LSTM(128, return_sequences=True, kernel_regularizer=l2_reg), merge_mode='sum')(cnn_out) # V3: Increased capacity & L2 reg
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - Hybrid V3')
    plt.ylabel('Loss (Huber)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_hybrid_v3.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_hybrid_v3.png")

        # --- BENCHMARKING REPORT ---
        print("\n\n--- BENCHMARKING REPORT (V3) ---")
        total_params = solar_model.count_params()
        model_size_mb = total_params * 4 / (1024**2)
        print(f"{'Model Name:':<25} Hybrid CNN-CycleLSTM-Attention ({SEASON} V3)")
