...
# # 72-Hour Renewable Energy Forecast (Transformer) - V2 Season-aware Model
# 
# ## Modifications for V2:
# 
# This notebook enhances the original Transformer model with several key improvements aimed at boosting predictive accuracy and ensuring robustness:
# 
# 1.  **Simplified Transformer Architecture:** The model's complexity has been reduced to better suit the univariate time series data. The number of transformer blocks is decreased from `4` to `2`, and the number of attention heads from `4` to `2`. This creates a more lightweight and efficient architecture, reducing the risk of overfitting and improving generalization.
# 
# 2.  **Positional Embedding:** A `TokenAndPositionEmbedding` layer has been introduced. The standard Transformer architecture is permutation-invariant and requires explicit positional information to understand the sequence order. This new layer adds positional embeddings to the input, ensuring the model is aware of the temporal dependencies in the data.
# 
...
if X_train.shape[0] > 0:
    solar_model = build_transformer_model(
        ...
        head_size=128, # V2: Reduced complexity
        num_heads=2,   # V2: Reduced complexity
        ff_dim=4,
        num_transformer_blocks=2, # V2: Reduced complexity
        mlp_units=[128],
        mlp_dropout=0.4,
        dropout=0.25,
    )
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - Transformer V2')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_transformer_v2.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_transformer_v2.png")
            
        # --- BENCHMARKING REPORT ---
        print("\n\n--- BENCHMARKING REPORT (V2) ---")
        total_params = solar_model.count_params()
        model_size_mb = total_params * 4 / (1024**2)
        print(f"{'Model Name:':<25} Transformer ({SEASON} V2)")
