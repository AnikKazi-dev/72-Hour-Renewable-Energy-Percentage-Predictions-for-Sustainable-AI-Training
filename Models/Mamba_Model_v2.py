...
# # 72-Hour Renewable Energy Forecast (Mamba) - V2 Improved Winter Model
# 
# ## Modifications for V2:
# 
# This notebook enhances the original Mamba model with several key improvements aimed at boosting predictive accuracy and ensuring robustness:
# 
# 1.  **Architectural Refinement:** The model's complexity has been reduced to better suit the univariate time series data. The number of Mamba layers is decreased from `3` to `2`, and the `d_model` from `128` to `64`. This creates a more lightweight and efficient architecture, reducing the risk of overfitting and improving generalization.
# 
# 2.  **Residual Connections:** Residual connections have been added around each `MambaBlock`. This is a standard practice in deep learning that helps with gradient flow and allows the model to learn more complex functions.
# 
# 3.  **Layer Normalization:** `LayerNormalization` has been added before each `MambaBlock`. This helps to stabilize the training process and can lead to better performance.
# 
...
from tensorflow.keras.layers import Input, Dense, Dropout, Conv1D, GlobalAveragePooling1D, LayerNormalization, Add
...
try:
    ...
    # Filter the loaded data to ensure it only contains months for the active season
...
class MambaBlock(tf.keras.layers.Layer):
    def __init__(self, d_model, d_state, d_conv, expand, **kwargs):
        ...
        self.in_proj = Dense(self.expand * self.d_model * 2, use_bias=False)
        self.conv1d = Conv1D(filters=self.expand * self.d_model, kernel_size=d_conv, padding='causal', groups=self.expand * self.d_model)
...
def build_mamba_model(input_shape, forecast_horizon, d_model=64, d_state=16, d_conv=4, expand=2, num_layers=2, mlp_units=[256], mlp_dropout=0.3):
    inputs = Input(shape=input_shape)
    x = inputs
    for _ in range(num_layers):
        res = x
        x = LayerNormalization()(x)
        x = MambaBlock(d_model, d_state, d_conv, expand)(x)
        x = Add()([res, x])
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - Mamba V2')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_mamba_v2.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_mamba_v2.png")
            
            # --- BENCHMARKING REPORT ---
            print("\n\n--- BENCHMARKING REPORT (V2) ---")
            total_params = solar_model.count_params()
            model_size_mb = total_params * 4 / (1024**2)
            print(f"{'Model Name:':<25} Mamba ({SEASON} V2)")
