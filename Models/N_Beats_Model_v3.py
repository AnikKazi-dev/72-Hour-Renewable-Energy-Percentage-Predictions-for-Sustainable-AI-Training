...
# # 72-Hour Renewable Energy Forecast (N-BEATS) - V3 Improved Winter Model
# 
# ## Modifications for V3:
# 
# This notebook enhances the V2 N-BEATS model with several key improvements aimed at boosting predictive accuracy and robustness:
# 
...
# 
# 2.  **L2 Regularization**: To prevent overfitting, L2 regularization (`kernel_regularizer`) has been added to all `Dense` layers within the N-BEATS blocks. This encourages the model to learn smaller, more generalizable weights.
# 
# 3.  **Increased Model Capacity**: The model's capacity has been significantly increased to capture more complex patterns in the data. The number of units in the dense layers of each N-BEATS block is increased from 128 to 256, and the `theta_dim` is also increased to 256.
# 
# 4.  **Increased Number of Blocks**: The number of N-BEATS blocks has been increased from 3 to 4. This allows the model to learn a more hierarchical representation of the time series, potentially improving its ability to model complex patterns.
...
# N-BEATS Block (V3 with L2 Regularization and increased capacity)
class NBeatsBlock(Layer):
    def __init__(self, units, theta_dim, num_layers, look_back, forecast_horizon, dropout_rate=0.2, **kwargs):
        ...
        self.fc_layers = [Dense(units, activation='relu', kernel_regularizer=l2_reg) for _ in range(num_layers)]
        self.dropout_layers = [Dropout(self.dropout_rate) for _ in range(num_layers)]
        
        self.theta_f = Dense(theta_dim, activation='linear', name='theta_f', kernel_regularizer=l2_reg)
        self.theta_b = Dense(theta_dim, activation='linear', name='theta_b', kernel_regularizer=l2_reg)
...
def build_nbeats_model(look_back, forecast_horizon, num_blocks=4, num_layers=4, units=256, theta_dim=256):
    """
    Builds the N-BEATS model (V3 with increased capacity).
    """
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - N-BEATS V3')
    plt.ylabel('Loss (Huber)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_nbeats_v3.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_nbeats_v3.png")
        ...
        print(f"{'Model Name:':<25} N-BEATS ({SEASON} V3)")
