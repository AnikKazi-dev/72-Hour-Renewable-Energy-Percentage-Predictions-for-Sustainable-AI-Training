...
# # 72-Hour Renewable Energy Forecast (DLinear) - V3 Improved Model
# 
# ## Modifications for V3:
# 
# This notebook enhances the V2 DLinear model with several key improvements aimed at boosting predictive accuracy and robustness:
# 
...
# 
# 2.  **L2 Regularization**: L2 regularization has been added to the `Dense` layers in the model. L2 regularization, also known as weight decay, penalizes large weights in the model. This helps to prevent overfitting by encouraging the model to learn smaller and more distributed weights, which can lead to better generalization on unseen data.
# 
# 3.  **Non-Linear Decomposition Head**: The single `Dense` layer for both the seasonal and trend components has been replaced with a small multi-layer perceptron (MLP) block (`Dense` -> `ReLU` -> `Dropout` -> `Dense`). This gives the model more capacity to learn potentially non-linear patterns within the decomposed trend and seasonal components, while still maintaining the simple and effective DLinear decomposition structure.
...
def build_dlinear_model(input_shape, forecast_horizon, kernel_size=25, mlp_units=512, mlp_dropout=0.2):
    inputs = Input(shape=input_shape)
    
    # Trend part
    trend_input = MovingAverage(kernel_size)(inputs)
    # V3: Non-linear head
    x_trend = Dense(mlp_units, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))(trend_input)
    x_trend = Dropout(mlp_dropout)(x_trend)
    trend_output = Dense(forecast_horizon, kernel_regularizer=tf.keras.regularizers.l2(0.001))(x_trend)
    
    # Seasonal part
    seasonal_input = inputs - trend_input
    # V3: Non-linear head
    x_seasonal = Dense(mlp_units, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(0.001))(seasonal_input)
    x_seasonal = Dropout(mlp_dropout)(x_seasonal)
    seasonal_output = Dense(forecast_horizon, kernel_regularizer=tf.keras.regularizers.l2(0.001))(x_seasonal)
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - DLinear V3')
    plt.ylabel('Loss (Huber)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_dlinear_v3.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_dlinear_v3.png")
        ...
        print(f"{'Model Name:':<25} DLinear ({SEASON} V3)")
