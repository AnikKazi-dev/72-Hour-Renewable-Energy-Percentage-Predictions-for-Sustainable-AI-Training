...
# # 72-Hour Renewable Energy Forecast (Cycle-LSTM) - V2 Improved Model
# 
# ## Modifications for V2:
# 
# This notebook enhances the original Cycle-LSTM model with several key improvements aimed at boosting predictive accuracy and modernizing the architecture:
# 
# 1.  **Standardized Bidirectional LSTM:** The manual implementation of the Cycle-LSTM has been replaced with the standard `tf.keras.layers.Bidirectional` wrapper. This improves code readability and aligns with Keras best practices while preserving the core bidirectional logic.
# 
# 2.  **Stacked LSTM Layers:** A second `Bidirectional(LSTM)` layer has been added to create a stacked architecture. This allows the model to learn more complex, hierarchical temporal patterns from the data, which can lead to improved performance.
# 
# 3.  **Refined Architecture:** The number of units in the LSTM and Dense layers has been adjusted to create a more balanced model. The LSTM layers now have `64` units, and the Dense layer has `100` units, providing a good trade-off between capacity and efficiency.
# 
# 4.  **Dynamic Learning Rate Scheduling:** The `ReduceLROnPlateau` callback has been integrated into the training process. This allows the learning rate to be adjusted dynamically based on the validation loss, helping the model to converge more effectively.
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - Cycle-LSTM V2')
    plt.ylabel('Loss (Mean Absolute Error)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_cycle_lstm_v2.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_cycle_lstm_v2.png")
        ...
        print(f"{'Model Name:':<25} Cycle-LSTM ({SEASON} V2)")
