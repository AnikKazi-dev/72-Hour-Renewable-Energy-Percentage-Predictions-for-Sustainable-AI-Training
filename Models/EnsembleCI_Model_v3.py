...
# # 72-Hour Renewable Energy Forecast (EnsembleCI) - V3 Improved Winter Model
# 
# ## Modifications for V3:
# 
# This notebook enhances the V2 EnsembleCI model with several key improvements aimed at boosting predictive accuracy and robustness:
# 
...
# 
# 2.  **L2 Regularization**: L2 regularization has been added to all `Conv1D`, `LSTM`, and `Dense` layers in the model. L2 regularization, also known as weight decay, penalizes large weights in the model. This helps to prevent overfitting by encouraging the model to learn smaller and more distributed weights, which can lead to better generalization on unseen data.
# 
# 3.  **Increased Model Capacity and Deeper Head**: To potentially capture more intricate patterns, the model's capacity has been increased. The number of filters/units in the `Conv1D` and `LSTM` layers are increased from `64` to `128`. The final dense head has also been made deeper and wider (`Dense(256)` -> `BN` -> `Dense(128)`) to allow for more complex combinations of the features from the two ensemble branches.
...
def build_ensemble_model(input_shape, forecast_horizon):
    ...
    shared_cnn = Conv1D(filters=128, kernel_size=3, activation='relu', padding='same', kernel_regularizer=l2_reg)(inputs) # V3: Increased capacity & L2 reg
    shared_cnn = BatchNormalization()(shared_cnn)
    shared_cnn = Dropout(0.2)(shared_cnn)
    
    # Model 1: CNN-LSTM
    cnn_lstm_out = Bidirectional(LSTM(128, return_sequences=True, kernel_regularizer=l2_reg))(shared_cnn) # V3: Increased capacity & L2 reg
    cnn_lstm_out = Bidirectional(LSTM(128, return_sequences=False, kernel_regularizer=l2_reg))(cnn_lstm_out) # V3: Increased capacity & L2 reg
    
    # Model 2: LSTM
    lstm_out = Bidirectional(LSTM(128, return_sequences=True, kernel_regularizer=l2_reg))(inputs) # V3: Increased capacity & L2 reg
    lstm_out = Bidirectional(LSTM(128, return_sequences=False, kernel_regularizer=l2_reg))(lstm_out) # V3: Increased capacity & L2 reg
    
    # Ensemble
    concatenated = Concatenate()([cnn_lstm_out, lstm_out])
    
    # V3: Deeper Dense head
    x = Dense(256, activation='relu', kernel_regularizer=l2_reg)(concatenated)
...
if solar_model and X_train.shape[0] > 0:
    ...
    plt.title(f'Model Loss During Training ({SEASON}) - EnsembleCI V3')
    plt.ylabel('Loss (Huber)')
    plt.xlabel('Epoch')
    plt.legend(loc='upper right')
    plt.grid(True)
    plt.savefig(f"training_loss_plot_{SEASON.lower()}_ensembleci_v3.png")
    ...
    if X_test.shape[0] > 0:
        ...
        if num_plots > 0:
            ...
            for i in range(num_plots):
                ...
                plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"forecast_examples_{SEASON.lower()}_ensembleci_v3.png")

        # --- BENCHMARKING REPORT ---
        print("\n\n--- BENCHMARKING REPORT (V3) ---")
        total_params = solar_model.count_params()
        model_size_mb = total_params * 4 / (1024**2)
        print(f"{'Model Name:':<25} EnsembleCI ({SEASON} V3)")
